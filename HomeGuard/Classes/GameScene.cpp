...
void GameScene::initToolBar(){
  ...
  //  auto moneyText = std::to_string(money);
	auto moneyText = StringUtils::format("%ld", money);
   ...
   // auto groupInfoText = std::to_string(this->npcNumber);
	auto groupInfoText = StringUtils::format("%d", npcNumber);
	...
	auto groupTotalText = StringUtils::format("%d", npcNumberCount);
